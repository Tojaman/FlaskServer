...
def download_online_model(url, dir_name, progress=gr.Progress()):
    try:
        progress(0, desc=f'[~] Downloading video model with name {dir_name}...')
        zip_name = url.split('/')[-1]
        extraction_folder = os.path.join(rvc_models_dir, dir_name)
        if os.path.exists(extraction_folder):
            raise gr.Error(f'Voice model directory {dir_name} already exists! Choose a different name for your video model.')
    ...
    with gr.Blocks(title='AICoverGenWebUI') as app:
        ...
        with gr.Tab("Generate"):
            ...
            with gr.Accordion('Voice conversion options', open=False):
                with gr.Row():
                    index_rate = gr.Slider(0, 1, value=0.5, label='Index Rate', info="Controls how much of the AI video's accent to keep in the vocals")
                    ...
                    with gr.Column():
                        ...
                        crepe_hop_length = gr.Slider(32, 320, value=128, step=1, visible=False, label='Crepe hop length', info='Lower values leads to longer conversions and higher risk of video cracks, but better pitch accuracy.')
        ...
        with gr.Tab('Download model'):
            ...
            with gr.Tab('From Public Index'):
                ...
                load_public_models_button = gr.Button(value='Initialize public models'
        ...
        with gr.Tab('Upload model'):
            ...
            gr.Markdown('- Upload zip file and give unique name for video')
